---
title: API Calls
description: Overview of REST API, HTTP methods, status codes, Swagger, FastAPI, and API utility tools.
navigation:
...
API creats endpoints specifled by paths `path/to/api/endpoint` and a function is declared at those endpoints in the backend servers and a client will make request to a backend server at aspecific IP address and port.
...
![FastAPI](/images/web/fastapi.svg){style="max-width: 192px; margin: 0 auto;"}
...
![FastAPI](/images/web/fastapi.png)

```python {title="backend/main.py"}{style="max-width: 400px; margin: 0 auto;"}
...
{{< callout context="caution" title="Tips" icon="outline/alert-triangle" >}}
