---
title: Docker
description: Introduction to Docker, containers, Dockerfile, Docker Compose, and networking.
navigation:
...
![Docker](/images/web/docker-icon.svg){width="192" class="mx-auto block bg-white p-4 rounded-xl my-8"}
...
![Kubernetes](/images/web/kubernetes.svg){width="192" class="mx-auto block bg-white p-4 rounded-xl my-8"}
...
Spawn a new process inside the sandbox. Usually the process will read the logs or `curl` to the process. It's usually used for specific operational task rather than repeated task.

```bash
docker exec -it my-container /bin/bash
```

- Flag `-i` for interactive
- Flag `-tty` for nicer terminal
- Flag `it` is for both

Note: It depend on the image. If it's a full linux distro, we can do a lot. Standard Alpine Linuy is 5MB and only have a lightweight /bin/sh
...
docker run -d --name es --net foodtrucks-net -p 9201:9200 -p 9300:9300 -e "discovery.type=single-node" docker.elastic.co/elasticsearch/elasticsearch:6.3.2
...
Mounting path creates vulerable on host machine, never run as root and don't mount at sensitive location. Consider read only mount.

```bash
docker run -v /home/user/configs:/app/configs:ro nginx
```

#### The Build Cache Layering

- Use multiple-stage build
- Every line in Dockerfile creates a layer. Dependency installation can be cached so that changing code woudn't have to reinstall the dependency.
