---
title: Python
description: Comprehensive guide to Python syntax, data structures, libraries, and tools.
navigation:
  icon: i-logos-python
---
...
::collapsible{name="Python Tools collapsible"}
...
::collapsible{name="Advanced Syntax"}
...
num = random.randint(1, n) # Generate random number between 1 and n

char.isalpha()  # Check if char is alphabet
char.isdigit()  # Check if char is number

char.islower()  # Check if lower case
char.isupper()

char.lower()    # Change to lower case
char.upper()
...
lst.sort()                 # Sort in place (Timsort: O(n log n), use merge sort for larger lists, insertion sort for small lists)
...
for k,v in d.items()       # View of key-value tuples
...
`CPython` cannot run multiple threads in parallel for CPU work. A GIL is a mutex that ensure only one thread execute python bytecode at a time, protecting memory management and race conditions.

- Use seperate processes (multiprocessing) / C extensions / NumPy
- Good for IO to avoid complex mess

Make a subprocess to run true parallelism. Async is multiple task on single thread.

::info
Python 3.13 introduce free-threading (no GIL) experimental
::
...
```py
a = [1, 2, 3]
a.append(4)   # same object, changed
```
...
```py
a = "hello"
a = a + " world"   # creates a NEW object 'a'
```
